...
from training.nn import Linear, Conv2d, AttentionOp, GroupNorm
from training.lora import LoraInjectedConv
...
class UNetBlock(torch.nn.Module):
    ...
    def forward(self, x, emb):
        ...
        if self.num_heads:
            if isinstance(self.qkv, LoraInjectedConv):
                q, k, v = self.qkv(self.norm2(x), emb).reshape(x.shape[0] * self.num_heads, x.shape[1] // self.num_heads, 3,
                                                          -1).unbind(2)
            else:
                q, k, v = self.qkv(self.norm2(x)).reshape(x.shape[0] * self.num_heads, x.shape[1] // self.num_heads, 3, -1).unbind(2)
            w = AttentionOp.apply(q, k)
            a = torch.einsum('nqk,nck->ncq', w, v)
            if isinstance(self.proj, LoraInjectedConv):
