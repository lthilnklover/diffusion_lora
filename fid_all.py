...
def calc(image_path, ref_path, num_expected, seed, batch, out_dir=None):
    ...
    if dist.get_rank() == 0:
        ...
        if out_dir is not None:
            iter_num = image_path.split('/')[-1]
            with open(os.path.join(out_dir, 'fid.txt'), 'a') as f:
                f.write(f"{iter_num}: {fid:g}\n")
    torch.distributed.barrier()
...
def main(image_paths, ref_path, num_expected, seed, batch):
    ...
    for image_path in path_list:
        calc(image_path, ref_path, num_expected, seed, batch, out_dir=image_paths)
